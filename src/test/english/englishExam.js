...
const parseLineBreaks = (text) => {
  ...
  const parseContent = (text) => {
    ...
    while ((match = regex.exec(text)) !== null) {
      ...
      const processedContent = parseLineBreaks(contentText); // <br> 태그 처리 추가
      parsedText = parsedText.replace(match[0], `<div style="border: 1px solid black; padding: 10px; margin: 5px 0;">${processedContent}</div>`);
    }
    return parsedText;
  };
    ...
    <div>
      {questions.map((questionData, questionIndex) => (
        <div key={questionIndex}>
          ...
          {parse(parseHeaders(parseTable(parseLongLine(parseLongLongLine(parseContent(questionData.question))))))}
